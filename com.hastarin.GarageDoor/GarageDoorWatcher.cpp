...
GarageDoorWatcher::GarageDoorWatcher(AllJoynBusAttachment^ busAttachment) :
...
GarageDoorWatcher::~GarageDoorWatcher()
{
    UnregisterFromBus();
}

void GarageDoorWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.hastarin.GarageDoor" };
...
void GarageDoorWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.hastarin.GarageDoor"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.hastarin.GarageDoor"),
            port);
        Added(this, args);
    }
...
void GarageDoorWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<GarageDoorWatcher>
        };
...
void GarageDoorWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_GarageDoorIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &GarageDoorWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void GarageDoorWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void GarageDoorWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
